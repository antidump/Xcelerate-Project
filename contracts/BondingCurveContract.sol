...
contract BondingCurveContract is Ownable, ReentrancyGuard {
    ...
    function buyTokens(address token, uint256 okbAmount) 
        external 
        payable 
        nonReentrant 
        ...
        returns (uint256) 
    {
        ...
        emit TokenBought(token, msg.sender, okbAmount, tokensOut, newPrice);
        ...
        returns (uint256) 
    {
        ...
        if (feeManager != address(0)) {
            ...
            payable(msg.sender).transfer(okbOut);
        }
        
        uint256 newPrice = BondingCurve.calculatePrice(curve.soldSupply);
        emit TokenSold(token, msg.sender, tokenAmount, okbOut, newPrice);
