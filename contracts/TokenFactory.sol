...
contract TokenFactory is Ownable, ReentrancyGuard {
    ...
    function createToken(
        ...
        // Initialize bonding curve for the new token
        (bool success, ) = bondingCurve.call(
            abi.encodeWithSignature("initializeCurve(address)", tokenAddress)
        );
        require(success, "Bonding curve initialization failed");
        
        address feeManager = registry.getFeeManager();
        if (feeManager != address(0) && msg.value > 0) {
            (bool feeSuccess, ) = feeManager.call{value: msg.value}("");
            require(feeSuccess, "Fee transfer failed");
        }
        
        emit TokenCreated(tokenAddress, msg.sender, name, symbol, INITIAL_SUPPLY);
